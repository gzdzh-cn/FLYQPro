...
func (s *sBaseOpenService) GetSetting(ctx context.Context, req *v1.GetSettingReq) (data interface{}, err error) {
	values := config.GetSettingValues(ctx, "siteName", "logo", "copyright")
	if values == nil {
		return nil, gerror.New("读取站点配置失败")
	}
	data = &types.CommonSetting{
...
func (s *sBaseOpenService) ServerInfo(ctx context.Context) (data interface{}, err error) {
	...
	values := config.GetSettingValues(ctx, "siteName")
	if values == nil {
		return nil, gerror.New("读取站点配置失败")
	}
