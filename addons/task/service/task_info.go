...
// ModifyBefore marks tasks created from the task addon as task-owned, so a
// plugin backup never mixes them with scheduled jobs registered by another addon.
func (s *TaskInfoService) ModifyBefore(_ g.Ctx, method string, param g.MapStrAny) (err error) {
	if method == "Add" && gconv.String(param["addonsName"]) == "" {
		param["addonsName"] = "task"
	}
	return nil
}
