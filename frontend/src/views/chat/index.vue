...
function messageStatusText(status: string) { return ({ sending: '发送中', sent: '发送成功', delivered: '发送成功', read: '已读', queued: '等待发送', failed: '发送失败' } as Record<string, string>)[status] || status }
...
function selectPeer(peer: Peer) { store.selectPeer(peer.deviceId); showPeerInfo.value = false; ChatService.EnsureConversation(peer.deviceId).then((id) => ChatService.ListMessages(id).then((messages) => { store.messages[id] = messages; return ChatService.MarkConversationRead(peer.deviceId) }).catch(() => undefined)) }
...
async function sendMessage() { if (!activePeer.value || !draft.value.trim()) return; try { await ChatService.SendMessage(activePeer.value.deviceId, draft.value.trim()); draft.value = '' } catch (error: any) { Message.error(error?.message || '发送失败') } }
