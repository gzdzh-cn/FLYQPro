...
function resetViewState() { relativePath.value = ''; search.value = ''; entries.value = []; selected.clear(); context.visible = false; context.entry = undefined; Object.keys(transfers).forEach((key) => delete transfers[key]) }
...
async function chooseRoot() { const path = await ChatService.PickDirectory(); if (!path) return; try { Object.assign(settings, await ChatService.SetSharedFolder(path, settings.enabled)); await refresh(); Message.success('共享目录已更新') } catch (error: any) { Message.error(error?.message || '设置共享目录失败') } }
...
async function renameEntry() { if (!context.entry) return; const name = window.prompt('请输入新名称', context.entry.name); if (!name) return; try { await ChatService.RenameSharedEntry(context.entry.relativePath, name); await refresh() } catch (error: any) { Message.error(error?.message || '重命名失败') } finally { closeContext() } }
