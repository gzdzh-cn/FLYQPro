	...
	<cl-crud ref="Crud">
		...
		<cl-row>
			...
			<el-dropdown trigger="hover" @command="onMoreCommand">
				...
				<template #dropdown>
					<el-dropdown-menu>
						<el-dropdown-item command="excel" v-if="service.customer_pro.clues_filter._permission.excel">
							导出Excel
						</el-dropdown-item>
						<el-dropdown-item command="distribute" :disabled="Table?.selection.length == 0"
							v-if="service.customer_pro.clues_filter._permission.distribute">
							转交
			...
			<el-dropdown trigger="click" @command="onSortCommand" :hide-on-click="false">
				<el-button plain size="small">
					...
					{{ sortFieldLabel ? sortFieldLabel + (sortOrder === 'ASC' ? ' ↑' : sortOrder === 'DESC' ? ' ↓' : '')
						: '排序' }}
				</el-button>
				<template #dropdown>
					<el-dropdown-menu class="sort-dropdown-menu">
						...
						<el-dropdown-item v-for="item in sortFieldOptions" :key="item.value"
							:command="'field:' + item.value" :class="{ 'is-active': sortField === item.value }">
							{{ item.label }}
							<el-icon v-if="sortField === item.value" style="margin-left: auto;">
								<Check />
							</el-icon>
						</el-dropdown-item>
						<el-dropdown-item divided class="sort-group-title" disabled>排序方式</el-dropdown-item>
						<el-dropdown-item command="order:ASC" :class="{ 'is-active': sortOrder === 'ASC' }">
							正序 ↑
							<el-icon v-if="sortOrder === 'ASC'" style="margin-left: auto;">
								<Check />
							</el-icon>
						</el-dropdown-item>
						<el-dropdown-item command="order:DESC" :class="{ 'is-active': sortOrder === 'DESC' }">
							倒序 ↓
							<el-icon v-if="sortOrder === 'DESC'" style="margin-left: auto;">
								<Check />
							</el-icon>
						</el-dropdown-item>
						<el-dropdown-item command="order:default"
							...
							默认
							<el-icon v-if="sortOrder === '' && sortField === ''" style="margin-left: auto;">
								<Check />
							</el-icon>
						</el-dropdown-item>
					</el-dropdown-menu>
		...
		<cl-row>
			<cl-table ref="Table" :border="true" @header-dragend="onHeaderDragend" @row-dblclick="onRowDblclick">
				<template #column-status="{ scope }">
					...
					<span style="color: #909399" v-if="scope.row.status == 3"> 已过滤 </span>
				</template>
		...
		<AdvSearchDialog v-model="advSearchVisible" :serviceGroup="serviceGroup" :tagTypeItems="tagTypeSearchItems"
			:showServiceGroup="true" :showServiceStatus="false" :showUpdateTime="false" @search="onAdvSearch"
		...
		<el-drawer v-model="cluesOpen" title="" direction="rtl" :size="drawerSize" :with-header="false"
			class="clues-drawer">
			<clues-info :cluesId="cluesId" mode="filter" :key="callKey" style="margin-top: -20px"
				@toggleFullscreen="toggleDrawerSize" @close="onCluesInfoClose" @tagTypesChanged="onTagTypesChanged" />
		</el-drawer>
	</cl-crud>
</template>
...
async function loadDictTranslators() {
	...
	translators.gender = (val: any) => { const map: Record<string, string> = { "0": "保密", "1": "男", "2": "女" }; return map[String(val)] || val || ""; };
	translators.cluesLevel = (val: any) => { const map: Record<string, string> = { "1": "A级", "2": "B级", "3": "C级", "4": "D级" }; if (val === null || val === undefined || val === "") return ""; return toValueArray(val).map((v: string) => map[v] || v).filter(Boolean).join("、"); };
	translators.sourceFrom = (val: any) => { const map: Record<string, string> = { "1": "手动录入", "2": "百度", "3": "抖音", "4": "53客服", "5": "小红书" }; if (val === null || val === undefined || val === "") return ""; const str = String(val); if (str.includes(",")) return str.split(",").map((v: string) => map[v.trim()] || v.trim()).filter(Boolean).join("、"); return map[str] || str; };
	translators.followupType = (val: any) => { const map: Record<string, string> = { "1": "待跟进", "2": "电话访谈", "21": "电话-无人接听", "22": "电话-拒接", "23": "电话-已接通", "3": "微信沟通", "31": "微信-待通过", "32": "微信-拒绝通过", "33": "微信-已通过", "4": "视频参观", "5": "预约参观", "6": "已参观" }; const childToParent: Record<string, string> = { "21": "2", "22": "2", "23": "2", "31": "3", "32": "3", "33": "3" }; if (val === null || val === undefined || val === "") return ""; const values = toValueArray(val); if (values.length === 1) return map[values[0]] || values[0]; if (values.length === 2) { const [first, second] = values; if (childToParent[second] === first) return (map[first] || first) + " / " + (map[second] || second); } return values.map((v: string) => map[v] || v).filter(Boolean).join("、"); };
	translators.householdType = (val: any) => { const map: Record<string, string> = { "1": "城镇", "2": "农村" }; if (val === null || val === undefined || val === "") return ""; return toValueArray(val).map((v: string) => map[v] || v).filter(Boolean).join("、"); };
	translators.education = (val: any) => { const map: Record<string, string> = { "1": "未知", "2": "初中", "3": "高中/中专/中技", "4": "大专/高技", "5": "本科" }; if (val === null || val === undefined || val === "") return ""; return toValueArray(val).map((v: string) => map[v] || v).filter(Boolean).join("、"); };
...
// ==================== 显示列设置 ====================
const COLUMN_CACHE_KEY = "cluesFilter_column_settings";
const COLUMN_PAGE_KEY = "cluesFilter_column_settings";
const DEFAULT_VISIBLE_COLUMNS = ["guestId", "keywords", "label", "mobile", "guestIpInfo", "createTime", "filterRemark", "lastFollowupTime", "lastFollowRemark"];
...
// cl-crud 配置（过滤线索：status=3）
const Crud = useCrud(
	{
		service: service.customer_pro.clues_filter,
		permission: { page: true, list: true, add: false, delete: true, update: false, info: false },
	...
	distributeFormRef.value?.open({
		...
		on: {
			...
			submit(data: { servicesId: any }, { done, close }: any) {
				service.customer_pro.clues_filter.distribute({ ids, servicesId: data.servicesId }).then(() => { ElMessage.success("转交完成"); close(); refresh(); }).catch((err) => { ElMessage.error(err.message); done(); });
			}
		}
	});
};
...
const toexcel = async (isCurrent: boolean) => {
	...
	const result = await service.customer_pro.clues.excel({ ...searchData.value, isCurrentPage: isCurrent });
	loading.close();
...
// 排序功能
const SORT_STORAGE_KEY = "cluesFilter_sort_state";
const SORT_PAGE_KEY = "cluesFilter_sort_state";
