				...
				<template #dropdown>
					<el-dropdown-menu>
						<el-dropdown-item command="distribute"
							...
							v-if="service.customer_pro.resource_comm._permission.distribute">
							转交
				...
				<template #slot-op="{ scope }">
					<div style="display:flex;flex-direction:row;flex-wrap:wrap;align-items:center;gap:12px;">
						<el-button text bg type="warning" @click="openTracks(scope.row)"
							v-if="service.customer_pro.resource_comm._permission.getTrackList">轨迹</el-button>
						<el-popconfirm title="确定认领该资源吗?" @confirm="claim(scope.row)">
							<template #reference>
								<el-button text bg type="info"
									v-permission="service.customer_pro.resource_comm._permission.claimClues">认领</el-button>
							</template>
						</el-popconfirm>
					</div>
				</template>
...
// ==================== 显示列设置 ====================
const COLUMN_CACHE_KEY = "resourceComm_column_settings";
const COLUMN_PAGE_KEY = "resourceComm_column_settings";
...
// 排序功能
const SORT_STORAGE_KEY = "resourceComm_sort_state";
const SORT_PAGE_KEY = "resourceComm_sort_state";
