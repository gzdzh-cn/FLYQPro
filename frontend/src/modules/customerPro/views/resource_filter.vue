<template>
	<cl-crud ref="Crud">
				...
				<template #dropdown>
					<el-dropdown-menu>
						<el-dropdown-item command="excel"
							v-if="service.customer_pro.resource_filter._permission.excel">
							导出Excel
						</el-dropdown-item>
						<el-dropdown-item command="distribute"
							:disabled="Table?.selection.length == 0"
							v-if="service.customer_pro.resource_filter._permission.distribute">
							转交
		...
		<AdvSearchDialog v-model="advSearchVisible" :serviceGroup="serviceGroup" :tagTypeItems="tagTypeSearchItems"
			:showServiceGroup="true" :showServiceStatus="false" :showUpdateTime="false"
		...
		<el-drawer v-model="cluesOpen" title="" direction="rtl" :size="drawerSize" :with-header="false"
			class="clues-drawer">
			<clues-info :cluesId="cluesId" mode="filter" :key="callKey" style="margin-top: -20px" @toggleFullscreen="toggleDrawerSize"
				@close="onCluesInfoClose" @tagTypesChanged="onTagTypesChanged" />
		</el-drawer>
	</cl-crud>
</template>

<script lang="ts" name="customer_pro-resource_filter" setup>
...
async function loadDictTranslators() {
	...
	translators.sourceFrom = (val: any) => { const map: Record<string, string> = { "1": "手动录入", "2": "百度", "3": "抖音", "4": "53客服", "5": "小红书", "6": "线索资源" }; if (val === null || val === undefined || val === "") return ""; const str = String(val); if (str.includes(",")) return str.split(",").map((v: string) => map[v.trim()] || v.trim()).filter(Boolean).join("、"); return map[str] || str; };
...
// ==================== 显示列设置 ====================
const COLUMN_CACHE_KEY = "resourceFilter_column_settings";
const COLUMN_PAGE_KEY = "resourceFilter_column_settings";
...
const Crud = useCrud(
	{
		service: service.customer_pro.resource_filter,
	...
	distributeFormRef.value?.open({
		...
		on: {
			...
			submit(data: { servicesId: any }, { done, close }: any) {
				service.customer_pro.resource_filter.distribute({ ids, servicesId: data.servicesId }).then(() => { ElMessage.success("转交完成"); close(); refresh(); }).catch((err) => { ElMessage.error(err.message); done(); });
			}
		}
	});
};
...
// 排序功能
const SORT_STORAGE_KEY = "resourceFilter_sort_state";
const SORT_PAGE_KEY = "resourceFilter_sort_state";
...
.owner-filter-group {
	margin-right: 8px;
	.el-button { font-size: 12px; padding: 5px 12px; }
}
</style>
<style>
