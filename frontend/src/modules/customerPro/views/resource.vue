<template>
	<cl-crud ref="Crud">
				...
				<template #dropdown>
					<el-dropdown-menu>
						...
						<el-dropdown-item command="distribute"
							...
							v-if="service.customer_pro.resource._permission.distribute">
							转交
							<el-tooltip content="请先勾选至少一条资源，再进行转交操作" placement="right">
				...
				<template #slot-op="{ scope }">
					<div style="
						...
						<el-button text bg type="warning" @click="openTracks(scope.row)"
							v-if="service.customer_pro.resource._permission.getTrackList">轨迹</el-button>
		...
		<cl-dialog title="跟进" v-model="visible">
			<sub-follow ref="FollowRef" :id="cluesId" :status="cluesStatus" dtype="resource" @cancel="cancel" />
		</cl-dialog>
		...
		<!-- 显示列设置弹窗 -->
		<el-dialog v-model="columnDialogVisible" title="显示列设置" width="500px" :close-on-click-modal="false" @open="openColumnDialog" class="column-dialog">
		...
		<el-drawer v-model="cluesOpen" title="" direction="rtl" :size="drawerSize" :with-header="false"
			class="clues-drawer">
			<clues-info :cluesId="cluesId" :key="callKey" style="margin-top: -20px" @toggleFullscreen="toggleDrawerSize"
...
async function loadDictTranslators() {
	...
	translators.sourceFrom = (val: any) => {
		const map: Record<string, string> = { "1": "手动录入", "2": "百度", "3": "抖音", "4": "53客服", "5": "小红书", "6": "线索资源" };
		if (val === null || val === undefined || val === "") return "";
	...
	try { const pl = await service.customer_pro.project.list(); const pm = new Map(pl.map((p: any) => [String(p.id), p.name])); translators.projectId = (val: any) => pm.get(String(val)) || val || ""; } catch {}
	try { const sl = await service.customer_pro.school.list(); const sm = new Map(sl.map((s: any) => [String(s.id), s.name])); translators.schoolId = (val: any) => sm.get(String(val)) || val || ""; } catch {}
	try { const ml = await service.customer_pro.majors.list({}); const mm = new Map(ml.map((m: any) => [String(m.id), m.name])); translators.majorsId = (val: any) => mm.get(String(val)) || val || ""; } catch {}
	try { const rl = await service.customer_pro.readtypes.list(); const rm = new Map(rl.map((r: any) => [String(r.id), r.name])); translators.majorsType = (val: any) => rm.get(String(val)) || val || ""; } catch {}
	try { const dl = await service.customer_pro.readdegree.list(); const dm = new Map(dl.map((r: any) => [String(r.id), r.name])); translators.degreeId = (val: any) => dm.get(String(val)) || val || ""; } catch {}
	try {
		...
		(infoList || []).forEach((it: any) => { const tid = String(it.typeId); if (!itemsByTypeId[tid]) itemsByTypeId[tid] = []; itemsByTypeId[tid].push(it); });
		publicTypes.forEach((t: any) => { const typeKey = t.key; if (!typeKey) return; const fieldName = TYPE_KEY_TO_FIELD[typeKey] || typeKey; const items = itemsByTypeId[String(t.id)] || []; if (!items.length) return; translators[typeKey] = (val: any, row: any) => { let raw = val; if (raw === undefined || raw === null || raw === "") raw = row[fieldName]; if ((raw === undefined || raw === null || raw === "") && row.labelJson) { raw = parseLabelJson(row.labelJson)[typeKey]; } if (!raw) return ""; return toValueArray(raw).map((v: string) => { const found = items.find((it: any) => String(it.value) === String(v) || String(it.name) === String(v)); return found?.name || v; }).join("、"); }; });
	} catch (e) { console.error("加载字典翻译器失败:", e); }
...
async function loadDynamicTagColumns() {
	try { const typeList: any[] = await service.dict.type.list({ order: "createTime", sort: "asc" }); const publicTypes = (typeList || []).filter((t: any) => String(t.isPublic) === "1"); if (!publicTypes.length) { dynamicTagColumns.value = []; return; } const columns: { label: string; prop: string; width?: number; isCustom?: boolean }[] = []; publicTypes.forEach((t: any) => { const typeKey = t.key; if (!typeKey) return; if (EDIT_INFO_FIXED_COLUMNS.some((c) => c.prop === typeKey || c.label === t.name)) return; const fieldName = TYPE_KEY_TO_FIELD[typeKey] || typeKey; columns.push({ label: t.name, prop: typeKey, width: 120, isCustom: !KNOWN_DB_FIELDS.has(fieldName) }); }); dynamicTagColumns.value = columns; } catch (e) { console.error("加载动态标签列失败:", e); dynamicTagColumns.value = []; }
}
	...
	try {
		...
		const items: any[] = publicTypes.map((t: any) => {
			...
			if (isHierarchical) {
				...
				return () => ({
					label: t.name, prop: typeKey + "Status",
					component: {
						name: "el-tree-select",
						props: {
							data: treeData, props: { value: "id", label: "label", children: "children" }, checkStrictly: true, renderAfterExpand: false,
							...isMulti ? { multiple: true, collapseTags: true, collapseTagsTooltip: true } : {},
							clearable: true, filterable: true, defaultExpandAll: true
						}
					}
				});
			}

			return () => ({ label: t.name, prop: typeKey + "Status", component: { name: "el-select", props: { clearable: true, multiple: isMulti }, options: dictItems.filter((it: any) => it.value != null).map((it: any) => ({ label: it.name, value: it.value })) } });
		});
		...
		const hardcodedOptions: Record<string, { label: string; options: { label: string; value: string }[] }> = {
			followupType: {
				label: "跟进方式",
				options: [
					{ label: "待跟进", value: "1" }, { label: "电话访谈", value: "2" },
					{ label: "电话-无人接听", value: "21" }, { label: "电话-拒接", value: "22" }, { label: "电话-已接通", value: "23" },
					{ label: "微信沟通", value: "3" },
					{ label: "微信-待通过", value: "31" }, { label: "微信-拒绝通过", value: "32" }, { label: "微信-已通过", value: "33" },
					{ label: "视频参观", value: "4" }, { label: "预约参观", value: "5" }, { label: "已参观", value: "6" }
				]
			}
		};
		for (const [typeKey, config] of Object.entries(hardcodedOptions)) {
			if (!dictTypeKeys.has(typeKey)) {
				const isMulti = MULTI_SELECT_KEYS.has(typeKey);
				items.push(() => ({ label: config.label, prop: typeKey + "Status", component: { name: "el-select", props: { clearable: true, multiple: isMulti }, options: config.options } }));
			}
		}
		return items;
	} catch (e) { console.error("加载标签类型搜索项失败:", e); return []; }
}

const allTableColumns = computed(() => {
	const specialCols: any[] = [
		{ label: "关键词", prop: "keywords", width: 200, formatter(row: any) { return h("span", { style: { color: row.keywords ? "var(--color-primary)" : undefined, cursor: "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }, title: row.keywords || "无", onClick: () => doClues(row) }, row.keywords || "无"); } },
		{ label: "标签", prop: "label", width: 260 },
		{ label: "手机号", prop: "mobile", width: 200 }
	];
	const fixedCols = EDIT_INFO_FIXED_COLUMNS.filter((c) => !SPECIAL_COLUMN_PROPS.has(c.prop)).map((c) => { const col: any = { label: c.label, prop: c.prop }; if (c.width) col.width = c.width; if (c.showOverflowTooltip) col.showOverflowTooltip = true; const translator = dictTranslators.value[c.prop]; const fieldName = TYPE_KEY_TO_FIELD[c.prop]; if (translator) { col.formatter = (row: any) => translator(fieldName ? row[fieldName] : row[c.prop], row); } if (c.prop === "guestId") { col.formatter = (row: any) => { return h("span", { style: { color: row.guestId ? "var(--color-primary)" : undefined, cursor: "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }, title: row.guestId || "无", onClick: () => doClues(row) }, row.guestId || "无"); }; } if (c.prop === "status") { col.formatter = (row: any) => { const s = row.status; if (s === 0 || s === "0") return "未成交"; if (s === 1 || s === "1") return "已成交"; return s ?? ""; }; } if (c.prop === "lastFollowRemark") { col.formatter = (row: any) => { const val = row.lastFollowRemark; if (!val) return ""; let str = String(val).replace(/<[^>]*>/g, ""); return str.length > 200 ? str.substring(0, 200) + "..." : str; }; } if (c.prop === "servicesName") { col.formatter = (row: any) => { return row.servicesName || ""; }; } return col; });
	const tagCols = dynamicTagColumns.value.map((c) => { const col: any = { label: c.label, prop: c.prop, width: c.width || 120 }; const fieldName = TYPE_KEY_TO_FIELD[c.prop] || c.prop; const translator = dictTranslators.value[c.prop]; if (translator) { col.formatter = (row: any) => { const val = row[fieldName]; if ((val === undefined || val === null || val === "") && row.labelJson) { return translator(parseLabelJson(row.labelJson)[c.prop], row); } return translator(val, row); }; } else if (c.isCustom) { col.formatter = (row: any) => { const raw = parseLabelJson(row.labelJson)[c.prop]; return raw ? String(raw) : ""; }; } return col; });
...
// ==================== 显示列设置 ====================
const COLUMN_CACHE_KEY = "resource_column_settings";
const COLUMN_PAGE_KEY = "resource_column_settings";
...
function getOriginalColumns(): ColumnSetting[] { const tagTypeProps = new Set([...dynamicTagColumns.value.map(c => c.prop), ...Object.keys(TYPE_KEY_TO_FIELD)]); return allTableColumns.value.map((col: any) => ({ prop: col.prop, label: col.label || col.prop, visible: true, width: col.width || undefined, showOverflow: col.prop !== "label", isTagType: tagTypeProps.has(col.prop), searchable: tagTypeProps.has(col.prop) ? false : undefined })); }
function loadColumnSettingsFromCache(): ColumnSetting[] | null { try { const cached = localStorage.getItem(COLUMN_CACHE_KEY); if (cached) return JSON.parse(cached); } catch {} return null; }
...
async function loadColumnSettingsFromServer(): Promise<ColumnSetting[] | null> { try { const res = await service.customer_pro.user_ui_config.getUiConfig({ pageKey: COLUMN_PAGE_KEY }); if (res?.configData) return JSON.parse(res.configData); } catch (e) { console.error("[显示列] 从后端加载失败:", e); } return null; }
async function saveColumnSettingsToServer(settings: ColumnSetting[]) { try { await service.customer_pro.user_ui_config.saveUiConfig({ pageKey: COLUMN_PAGE_KEY, configData: JSON.stringify(settings) }); } catch (e) { console.error("[显示列] 保存到后端失败:", e); } }
...
function applyColumns(settings: ColumnSetting[]) { const settingsMap = new Map(settings.map((s) => [s.prop, s])); const sortedProps = settings.filter((s) => s.visible).map((s) => s.prop); const propColMap = new Map<string, any>(); allTableColumns.value.forEach((col: any) => { if (col.prop) propColMap.set(col.prop, col); }); const newColumns: any[] = [{ type: "selection" }]; sortedProps.forEach((prop, index) => { const col = propColMap.get(prop); if (col) { const { hidden, orderNum, ...rest } = col; const s = settingsMap.get(prop); if (s?.width) rest.width = s.width; if (s?.showOverflow !== false) { rest.className = (rest.className ? rest.className + ' ' : '') + 'overflow-hidden'; } newColumns.push({ ...rest, orderNum: index + 1 }); } }); settings.filter((s) => !s.visible).forEach((s) => { const col = propColMap.get(s.prop); if (col) newColumns.push({ ...col, hidden: true }); }); Table.value?.setColumns(newColumns); }
function onHeaderDragend(newWidth: number, oldWidth: number, column: any) { const prop = column.property; if (!prop) return; const cached = loadColumnSettingsFromCache(); if (cached) { const item = cached.find((s) => s.prop === prop); if (item) { item.width = Math.round(newWidth); saveColumnSettingsToCache(cached); saveColumnSettingsToServer(cached); } } }
function openColumnDialog() { const original = getOriginalColumns(); const cached = loadColumnSettingsFromCache(); if (cached && cached.length) { const originalMap = new Map(original.map((s) => [s.prop, s])); const result: ColumnSetting[] = []; cached.forEach((c) => { const orig = originalMap.get(c.prop); if (orig) { result.push({ ...orig, ...c, label: orig.label }); originalMap.delete(c.prop); } }); originalMap.forEach((orig) => { result.push({ ...orig, visible: false }); }); columnSettings.value = result; } else { columnSettings.value = original.map((col) => ({ ...col })); } }
...
async function resetColumnSettings() { removeColumnSettingsCache(); const original = getOriginalColumns(); const defaultVisible = new Set(DEFAULT_VISIBLE_COLUMNS); const orderedVisible = DEFAULT_VISIBLE_COLUMNS.map((prop) => original.find((c) => c.prop === prop)).filter(Boolean).map((c) => ({ ...c!, visible: true })); const hiddenCols = original.filter((c) => !defaultVisible.has(c.prop)).map((c) => ({ ...c, visible: false })); columnSettings.value = [...orderedVisible, ...hiddenCols]; await saveColumnSettings(columnSettings.value); applyColumns(columnSettings.value); updateAdvSearchItems(); }
async function restoreColumnSettings() { const original = getOriginalColumns(); let cached = loadColumnSettingsFromCache(); if (!cached || !cached.length) { const serverData = await loadColumnSettingsFromServer(); if (serverData && serverData.length) { cached = serverData; saveColumnSettingsToCache(cached); } } if (!cached || !cached.length) { const defaultVisible = new Set(DEFAULT_VISIBLE_COLUMNS); const orderedVisible = DEFAULT_VISIBLE_COLUMNS.map((prop) => original.find((c) => c.prop === prop)).filter(Boolean).map((c) => ({ ...c!, visible: true })); const hiddenCols = original.filter((c) => !defaultVisible.has(c.prop)).map((c) => ({ ...c, visible: false })); cached = [...orderedVisible, ...hiddenCols]; } const originalMap = new Map(original.map((s) => [s.prop, s])); const cachedProps = new Set(cached.map((s) => s.prop)); const newSettings = [...cached]; originalMap.forEach((orig, prop) => { if (!cachedProps.has(prop)) { newSettings.push({ ...orig, visible: false }); } else { const existing = newSettings.find((s) => s.prop === prop); if (existing) { if (existing.showOverflow === undefined) { existing.showOverflow = orig.showOverflow; } if (existing.isTagType === undefined && orig.isTagType !== undefined) { existing.isTagType = orig.isTagType; } if (existing.searchable === undefined && orig.searchable !== undefined) { existing.searchable = orig.searchable; } } } }); const filtered = newSettings.filter((s) => originalMap.has(s.prop)); saveColumnSettingsToCache(filtered); await saveColumnSettingsToServer(filtered); columnSettings.value = filtered; applyColumns(filtered); updateAdvSearchItems(); }
...
const randomForm = ref<{ randomNum: number; dateTime: string }>({ randomNum: 1000, dateTime: "" });
const randomData = async () => {
	await (service.customer_pro.resource as any).randomData(randomForm.value);
...
const openFollow = (row: any) => { cluesId.value = row.id; cluesStatus.value = row.status; visible.value = true; };
const cancel = () => { visible.value = false; refresh(); };

// 线索详情抽屉
const cluesOpen = ref(false);
const drawerSize = ref("80%");
const doClues = (row: any) => { cluesId.value = row.id; callKey.value++; drawerSize.value = "80%"; cluesOpen.value = true; };
const onRowDblclick = (row: any) => { doClues(row); };
...
const openOrderAdd = async (row: any) => {
	...
	OrderFormRef.value?.open({
		title: "线索成交",
		items: [
			{ type: "tabs", props: { labels: [{ label: "基础信息", value: "base" }, { label: "个人信息", value: "person" }, { label: "收款信息", value: "financial" }] } },
			{ label: "学生名称", prop: "name", span: 8, component: { name: "el-input" }, required: true, group: "base" },
			{ label: "学生电话", prop: "mobile", span: 8, component: { name: "el-input" }, required: true, group: "base" },
			{ label: "接待人员", prop: "receiver", span: 8, component: { name: "el-input" }, required: true, group: "base" },
			{ label: "身份证", prop: "idcardNumber", span: 8, component: { name: "el-input" }, required: true, group: "base" },
			{ label: "性别", prop: "gender", span: 8, component: { name: "el-select", options: [{ label: "保密", value: "0" }, { label: "男", value: "1" }, { label: "女", value: "2" }] }, required: true, group: "base" },
			{ label: "意向院校", prop: "schoolId", span: 8, component: { name: "slot-schoolId" }, group: "base" },
			{ label: "意向专业", prop: "majorsId", span: 8, component: { name: "slot-majorsId" }, group: "base" },
			{ label: "报读类型", prop: "majorsType", span: 8, component: { name: "el-select", options: [] }, required: true, group: "base" },
			{ label: "报读层次", prop: "degreeId", span: 8, component: { name: "el-select", options: [] }, required: true, group: "base" },
			{ label: "通讯地址", prop: "address", span: 24, component: { name: "el-input" }, group: "person" },
			{ label: "紧急联系人", prop: "emergencyContact", span: 12, component: { name: "el-input" }, group: "person" },
			{ label: "紧急联系人电话", prop: "emergencyMobile", props: { labelWidth: "130px" }, span: 12, component: { name: "el-input" }, group: "person" },
			{ label: "微信", prop: "wechat", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "民族", prop: "nation", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "籍贯", prop: "nativePlace", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "政治面貌", prop: "politicsStatus", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "户口性质", prop: "householdType", span: 8, component: { name: "el-select" }, group: "person" },
			{ label: "户口所在地", prop: "householdAddress", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "是否应届", prop: "freshman", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "当前学历", prop: "education", span: 8, component: { name: "el-select" }, group: "person" },
			{ label: "毕业学校", prop: "graduatedSchool", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "毕业时间", prop: "graduatedDate", span: 8, component: { name: "el-input" }, group: "person" },
			{ label: "备注", prop: "remark", span: 24, component: { name: "el-input", props: { type: "textarea", rows: 4 } }, group: "base" },
			{ label: "校方收定位金", prop: "schoolPayment", span: 8, value: 0.0, component: { name: "el-input-number", props: { precision: 2, step: 0.1 } }, group: "financial" },
			{ label: "自收定位金", prop: "teamsPayment", span: 8, value: 0.0, component: { name: "el-input-number", props: { precision: 2, step: 0.1 } }, group: "financial" },
			{ label: "支付编号", prop: "serial", span: 8, component: { name: "el-input" }, group: "financial" },
			{ label: "缴费凭证", prop: "voucher", span: 8, component: { name: "cl-upload" }, group: "financial" }
		],
		form: { ...cluesOne },
		on: {
			async open() {
				...
				OrderFormRef.value?.setOptions("majorsType", majorsTypeList.map((e: any) => ({ label: e.name, value: e.id })));
				const degreeList = await service.customer_pro.readdegree.list();
				OrderFormRef.value?.setOptions("degreeId", degreeList.map((e: any) => ({ label: e.name, value: e.id })));
				OrderFormRef.value?.setOptions("householdType", [{ label: "城镇", value: "1" }, { label: "农村", value: "2" }]);
				OrderFormRef.value?.setOptions("education", [{ label: "未知", value: "1" }, { label: "初中", value: "2" }, { label: "高中/中专/中技", value: "3" }, { label: "大专/高技", value: "4" }, { label: "本科", value: "5" }]);
			},
			submit(data: any, { close, done }: any) {
				data.cluesId = cluesId.value;
				data.servicesId = row.servicesId;
				data.projectId = row.projectId;
				service.customer_pro.order.add({ ...data }).then(() => { done(); close(); refresh(); }).catch((err: any) => { done(); ElMessage.error(err.message); close(); });
			}
		}
	});
};
...
const openDistribute = async () => {
	groupList.value = []; distributeFormRef.value?.setForm("groupId", null); kfList.value = []; distributeFormRef.value?.setForm("servicesId", null);
	...
	distributeFormRef.value?.open({
		...
		on: {
			...
			submit(data: { servicesId: any }, { done, close }: any) {
				service.customer_pro.resource.distribute({ ids, servicesId: data.servicesId }).then(() => { ElMessage.success("转交完成"); close(); refresh(); }).catch((err) => { ElMessage.error(err.message); done(); });
			}
		}
	});
};
...
const projectId = ref();
const projectChange = (v: any) => { groupList.value = []; distributeFormRef.value?.setForm("groupId", null); kfList.value = []; distributeFormRef.value?.setForm("servicesId", null); projectId.value = v; getGroupList(v); };
const groupChange = (v: any) => { kfList.value = []; distributeFormRef.value?.setForm("servicesId", null); getKfList(v, projectId.value); };
const groupList = ref();
...
const getSchoolList = async () => { schoolList.value = await service.customer_pro.school.list(); };
const schoolChange = async (v: any) => { majorsList.value = []; OrderFormRef.value?.setForm("majorsId", null); getMajorList(v); };
const getMajorList = async (v: any) => { majorsList.value = await service.customer_pro.majors.list({ schoolId: v }); };

const serviceGroup = ref();
const getServiceGroup = async () => { const list = await service.customer_pro.project_group.list(); serviceGroup.value = list.map((item: any) => ({ label: item.name, value: item.id })); };

const onMoreCommand = (command: string) => { if (command === "excel") openExcelDialog(); else if (command === "distribute") openDistribute(); };
...
const getUserInfo = async () => { isAdmin.value = user.info.roleIds?.split(",").includes("1"); };

// 导出Excel
const openExcelDialog = () => { openExcel.value = true; };
...
const toexcel = async (isCurrent: boolean) => {
	...
	const result = await service.customer_pro.resource.excel({ ...searchData.value, isCurrentPage: isCurrent });
	loading.close();
	if (result.status == 1) { ElMessage.warning(result.msg); } else { ElMessage.success("导出任务已启动，请等待完成"); excelDownRef.value.refresh(); }
};
watch(() => openExcel.value, (val) => { if (!val) { excelDownRef.value.stopInterval(); } });

// 排序功能
const SORT_STORAGE_KEY = "resource_sort_state";
const SORT_PAGE_KEY = "resource_sort_state";
